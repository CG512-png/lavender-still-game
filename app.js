(() => {
  ...
  grad.addColorStop(0,'rgba(255,255,255,1)'); grad.addColorStop(.18,'rgba(255,244,190,.98)'); grad.addColorStop(.55,'rgba(255,210,90,.35)'); grad.addColorStop(1,'rgba(255,180,40,0)');
  g.fillStyle = grad; g.fillRect(0,0,64,64);
  const texture = new THREE.CanvasTexture(glowCanvas);
  const mat = new THREE.PointsMaterial({ size: 0.55, map: texture, transparent: true, opacity: 0.95, vertexColors: true, blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true });
  ...
  const sg = starCanvas.getContext('2d'); sg.translate(64,64); sg.fillStyle='#fff3a5'; sg.shadowColor='#ffd86b'; sg.shadowBlur=24;
  sg.beginPath(); for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5; const r=i%2===0?44:18; sg.lineTo(Math.cos(a)*r,Math.sin(a)*r);} sg.closePath(); sg.fill();
  ...
  const SNOW = 850, snowPos = new Float32Array(SNOW*3);
  ...
  const fireworks=[];
  function firework(){
    const n=180, p=new Float32Array(n*3), v=[]; const ox=(Math.random()-.5)*20, oy=5+Math.random()*12;
    for(let i=0;i<n;i++){p[i*3]=ox;p[i*3+1]=oy;p[i*3+2]=(Math.random()-.5)*4; const a=Math.random()*Math.PI*2,s=.07+Math.random()*.28, up=(Math.random()-.2)*.2;v.push(new THREE.Vector3(Math.cos(a)*s,Math.sin(a)*s+up,(Math.random()-.5)*s));}
    const gg=new THREE.BufferGeometry();gg.setAttribute('position',new THREE.BufferAttribute(p,3));
    const mm=new THREE.PointsMaterial({color:paletteSets[themeIndex][Math.floor(Math.random()*4)],size:.24,transparent:true,opacity:1,blending:THREE.AdditiveBlending,depthWrite:false});
    const obj=new THREE.Points(gg,mm);scene.add(obj);fireworks.push({obj,v,life:1});
  }
  ...
  let gathered=false, handX=0, handY=0, targetRotX=0, targetRotY=0, currentRotX=0, currentRotY=0;
  ...
  function onResults(results){
    ...
    if(!hands || !hands.length){gestureEl.textContent='手势：未检测到手'; return;}
    const lm=hands[0]; handX=1-lm[9].x; handY=lm[9].y;
    const thumb=Math.abs(lm[4].x-lm[3].x)>.045;
    ...
    const palmOpen=count>=4;
    const fist=count<=1 && Math.hypot(lm[8].x-lm[0].x,lm[8].y-lm[0].y)<.42;

    if(fist){gathered=true; targetRotY=(handX-.5)*1.9; targetRotX=(handY-.5)*1.0; gestureEl.textContent='手势：✊ 聚合 / 旋转';}
    else if(palmOpen){gathered=false; gestureEl.textContent='手势：🖐 星尘炸散';}
    else gestureEl.textContent='手势：'+count+' 指';

    if(count===1 && !oneLatch){themeIndex=(themeIndex+1)%paletteSets.length;applyTheme();oneLatch=true;} if(count!==1)oneLatch=false;
    if(count===2 && !twoLatch){snow.visible=!snow.visible;twoLatch=true;} if(count!==2)twoLatch=false;
    if(count===3 && !threeLatch){firework();setTimeout(firework,120);setTimeout(firework,240);threeLatch=true;} if(count!==3)threeLatch=false;
  }

  async function initHands(){
    try{
      if(!window.Hands || !window.Camera) throw new Error('MediaPipe CDN 加载失败，请检查网络。');
      ...
      await cam.start(); statusEl.textContent='摄像头已连接 · 手势识别运行中';
    }catch(err){console.error(err);statusEl.textContent='手势识别未启动';errorEl.textContent=err?.message||'请允许摄像头权限，并使用 HTTPS 打开页面。';}
  }
  initHands();
  ...
  function animate(){
    ...
    const pos=geom.attributes.position.array, target=gathered?treeTargets:burstTargets;
    const ease=gathered?.075:.035;
    for(let i=0;i<COUNT;i++){
      const i3=i*3; const wobble=gathered?Math.sin(t*1.7+phases[i])*.018:Math.sin(t*.8+phases[i])*.045;
      pos[i3]+=(target[i3]-pos[i3])*ease+wobble;
      pos[i3+1]+=(target[i3+1]-pos[i3+1])*ease;
      pos[i3+2]+=(target[i3+2]-pos[i3+2])*ease+wobble*.6;
    }
    geom.attributes.position.needsUpdate=true;
    currentRotX+=(targetRotX-currentRotX)*.06; currentRotY+=(targetRotY-currentRotY)*.06;
    treeGroup.rotation.x=currentRotX; treeGroup.rotation.y=currentRotY + t*.06;
