...
function firework(){
  ...
  for(let i=0;i<n;i++) {
    ...
    const a=Math.random()*Math.PI*2,s=.07+Math.random()*.28;
    v.push(new THREE.Vector3(Math.cos(a)*s,Math.sin(a)*s+(Math.random()-.2)*.2,(Math.random()-.5)*s));
  }
...
function drawHand(lm){
  ...
  for(const [a,b] of connections){
    handCtx.beginPath(); handCtx.moveTo(lm[a].x*640,lm[a].y*480); handCtx.lineTo(lm[b].x*640,lm[b].y*480); handCtx.stroke();
  }
...
function customThreeFinger(lm){
  if(!lm) return false;
  const i=isExtended(lm,8,6), m=isExtended(lm,12,10), r=isExtended(lm,16,14), p=isExtended(lm,20,18);
  return i && m && r && !p;
}

function handleGesture(name,lm){
  if(name===stableGesture) stableFrames++; else { stableGesture=name; stableFrames=1; }
  if(stableFrames < 2) return;

  if(name==='Closed_Fist') {
    gathered=true;
    if(lm){ targetRotY=((1-lm[9].x)-.5)*2.2; targetRotX=(lm[9].y-.5)*1.2; }
    gestureEl.textContent='手势：✊ Closed Fist · 聚合 / 旋转';
  } else if(name==='Open_Palm') {
    gathered=false;
    gestureEl.textContent='手势：🖐 Open Palm · 粒子散开';
  } else if(name==='Pointing_Up') {
    gestureEl.textContent='手势：☝ Pointing Up · 切换主题';
    if(lastGesture!=='Pointing_Up'){ themeIndex=(themeIndex+1)%palettes.length; applyTheme(); }
  } else if(name==='Victory') {
    gestureEl.textContent='手势：✌ Victory · 飘雪';
    if(lastGesture!=='Victory') snow.visible=!snow.visible;
  } else if(name==='Three_Fingers') {
    gestureEl.textContent='手势：🤟 三指 · 烟花';
    if(lastGesture!=='Three_Fingers'){ firework(); setTimeout(firework,120); setTimeout(firework,240); }
  } else {
    gestureEl.textContent='手势：检测到手 · '+name;
  }
  lastGesture=name;
}

// ---------- New MediaPipe Tasks Vision pipeline ----------
let recognizer=null, stream=null, running=false, lastVideoTime=-1;

async function startGestureControl(){
  if(running) return;
  startBtn.disabled=true;
  errorEl.textContent='';
  statusEl.textContent='正在加载新版手势模型…';
  try {
    ...
    statusEl.textContent='模型已加载，正在请求摄像头权限…';
    ...
    statusEl.textContent='摄像头已连接 · 把整只手放进右上角画面';
    ...
    statusEl.textContent='手势启动失败';
    errorEl.textContent=(err?.message||String(err))+'；请确认摄像头权限已允许。';
  }
}
...
function predictLoop(){
  ...
  try {
    if(video.readyState>=2 && video.currentTime!==lastVideoTime){
      ...
      drawHand(lm||null);
      if(lm){
        let name=result.gestures?.[0]?.[0]?.categoryName || 'None';
        const score=result.gestures?.[0]?.[0]?.score || 0;
        if(customThreeFinger(lm)) name='Three_Fingers';
        if(score<0.45 && name!=='Three_Fingers') name='None';
        handleGesture(name,lm);
      } else {
        stableGesture='None'; stableFrames=0; lastGesture='None';
        gestureEl.textContent='手势：摄像头正常 · 未检测到手';
      }
    }
  } catch(err) {
    console.warn('gesture frame error',err);
    statusEl.textContent='摄像头已开启 · 某一帧识别失败，正在继续';
  }
  requestAnimationFrame(predictLoop);
}
...
function animate(){
  ...
  for(let i=0;i<COUNT;i++){
    const k=i*3,w=gathered?Math.sin(t*1.7+phases[i])*.018:Math.sin(t*.8+phases[i])*.045;
    pos[k]+=(target[k]-pos[k])*ease+w;
    pos[k+1]+=(target[k+1]-pos[k+1])*ease;
    pos[k+2]+=(target[k+2]-pos[k+2])*ease+w*.6;
  }
  ...
  for(let i=fireworks.length-1;i>=0;i--){
    ...
    for(let j=0;j<f.v.length;j++){
      const k=j*3;
      a[k]+=f.v[j].x; a[k+1]+=f.v[j].y; a[k+2]+=f.v[j].z;
      f.v[j].y-=.0028;
    }
    f.obj.geometry.attributes.position.needsUpdate=true;
    f.obj.material.opacity=Math.max(0,f.life);
    if(f.life<=0){ scene.remove(f.obj); f.obj.geometry.dispose(); f.obj.material.dispose(); fireworks.splice(i,1); }
  }
